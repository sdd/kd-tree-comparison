...
use kiddo_v6::stem_strategies::eytzinger_pf::EytzingerPf;
...
macro_rules! bench_f64 {
    ($group:ident, $a:ty, $t:ty, $k:tt, $idx: ty, $size:tt, $subtype: expr) => {
        bench_query_nearest_one_float::<f64, $t, EytzingerPf<$k, 8>, $k>(
            &mut $group,
            $size,
            QUERY_POINTS_PER_LOOP,
            &format!("kiddo_v6_vecofarenas_eytzinger_pf {}", $subtype),
        );
    };
}
...
macro_rules! bench_f32 {
    ($group:ident, $a:ty, $t:ty, $k:tt, $idx: ty, $size:tt, $subtype: expr) => {
        bench_query_nearest_one_float::<f32, $t, EytzingerPf<$k, 4>, $k>(
            &mut $group,
            $size,
            QUERY_POINTS_PER_LOOP,
            &format!("kiddo_v6_vecofarenas_eytzinger_pf {}", $subtype),
