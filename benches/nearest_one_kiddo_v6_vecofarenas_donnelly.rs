...
use kiddo_v6::stem_strategies::{Donnelly, SimdPrune};
...
macro_rules! bench_f64 {
    ($group:ident, $a:ty, $t:ty, $k:tt, $idx: ty, $size:tt, $subtype: expr) => {
        bench_query_nearest_one_float::<f64, $t, Donnelly<3, 64, 8, $k>, $k>(
            &mut $group,
            $size,
            QUERY_POINTS_PER_LOOP,
            &format!("kiddo_v6_vecofarenas_donnelly {}", $subtype),
        );
    };
}
...
macro_rules! bench_f32 {
    ($group:ident, $a:ty, $t:ty, $k:tt, $idx: ty, $size:tt, $subtype: expr) => {
        bench_query_nearest_one_float::<f32, $t, Donnelly<4, 64, 4, $k>, $k>(
            &mut $group,
            $size,
            QUERY_POINTS_PER_LOOP,
            &format!("kiddo_v6_vecofarenas_donnelly {}", $subtype),
