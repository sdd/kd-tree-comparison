...
use nabo::{CandidateContainer, KDTree, NotNan, Parameters};
...
const NUM_NEAREST: usize = 3000;
const DIST_NEAREST: A = 0.01;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let nabo_parameters = Parameters {
        allow_self_match: true,
        epsilon: A::zero(),

        // Nabo works with normalized radius, rather than dist according to the metric.
        // We need to convert DIST_NEAREST so that it performs the same query as the other libraries
        // being benchmarked
        max_radius: 0.01.sqrt(),
        sort_results: true,
    };
    ...
    for (idx, query_point) in query_points.iter().enumerate() {
        ...
        let res = kiddo_v3_immutable.nearest_n_within::<SquaredEuclidean>(query_point, DIST_NEAREST, NUM_NEAREST, true);
        ...
        let res = kiddo_v3_std.nearest_n_within::<SquaredEuclidean>(query_point, DIST_NEAREST, NUM_NEAREST, true);
        let res_to_string = res.iter().map(|nn|nn.item.to_string()).collect::<Vec<_>>().join("-");
        ...
        let res = nabo.knn_advanced(NUM_NEAREST as u32, &P(nabo_query_point), CandidateContainer::Linear, &nabo_parameters, None);
